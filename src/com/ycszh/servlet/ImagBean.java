...
public class ImagBean {
	...
	public void writeImage(HttpServletRequest request,
		...
		try {
			...
			InputStream in = null;
			String xh = request.getParameter("xh") == null ? "" : request
					.getParameter("xh").trim();
			if (!xh.equals("")) {
				...
				if (conn != null) {
					...
					String sql = "";
					if (temp != null && temp.equals("photoLog")) {
						sql = "select photo from  dzda_jsz_daxxb_photo_log where id='" + xh + "'";
					} else {
						sql = "select photo from  dzda_jsz_daxxb_photo where xh='" + xh + "'";
					}
