	...
	public void writeImage(HttpServletRequest request,
		...
		try {
			...
			if (!pid.equals("")) {
				...
				conn = DriverManager
						.getConnection(
								"dbc:oracle:thin:@(DESCRIPTION = (ADDRESS_LIST = (ADDRESS = (PROTOCOL = TCP)(HOST =10.42.7.17)(PORT =1521)))(CONNECT_DATA =(SID =dowdb)))",
								"cgs", "cheguan69370");
				if (conn != null) {
					String sql = "";
					sql = "select photo_jpg from PHOTO_T where photo_no='"
							+ pid + "'";
					...
					{
						sql = "select photo_jpg from ID_PHOTO_T where id_no='"
							+ pid + "'";
						st = conn.createStatement();
						rs = st.executeQuery(sql);
						if (rs.next()) {
