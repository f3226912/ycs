...
package com.ycszh.util.barcode;
...
public class BarCodeServlet extends HttpServlet
{
    ...
    private BarCode getBarCode(Map<String, String> barCodeParam)
    {
        if(barcode == null)
            barcode = new BarCode();
        try
        {
        	  Object[] keySet = barCodeParam.keySet().toArray();
        	  if(barCodeParam.containsKey("width") && barCodeParam.containsValue("height")){
        		  setParameter("width", barCodeParam.get("width"));
    			  setParameter("height", barCodeParam.get("height"));
    			  setParameter("autoSize", "n");
        	  }
        	  for(int i = 0; i < barCodeParam.size(); i++){
        		  if("width".equals(keySet[i].toString()) || "height".equals(keySet[i].toString())){
        			  continue;
        		  }
        		  setParameter(keySet[i].toString(), barCodeParam.get(keySet[i]).toString());
        	  }
