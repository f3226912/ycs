...
$(document).ready(function(){
	...
	$("#btnsplx").click(function(e){
	    ...
	    if(ywlxhtml == null || ywlxhtml == ''){
	    	var list = getYwlx_wzp('', '', 'JDCYWSL', 'VEH_YWLX');
 ...
 * @return {TypeName} 
 */
function getYwlx_wzp(dmz, dmms2, dmlb, veh_drv){
	var list ;
	$.ajax({
		cache:false,
		async:false,
		url:path+'/wpz_ajax/wpzajax_getYwlxYwyy.action',
		type:'post',
...
function createYwlx(list){
	...
	if(list != null && list.length > 0){
		...
		var lastlist = getYwlx_wzp('',tempobj.dmz, 'JDCYWSL', 'VEH_YWYY');
		if(lastlist != null){
			cha = 2;
		}
		for(var i = 0; i < list.length; i++){
			ischeck = "";
			var obj = list[i];
			var list2 = getYwlx_wzp('',obj.dmz, 'JDCYWSL', 'VEH_YWYY');
			if(list2 != null){
...
//				}
		...
		var hphmval = $("#hphmval").val();
		var hpzlval = $("#hpzlval").val();
		var clsbdh = $("#ptdoclsbdh").val();
		if(hphmval==null||hphmval==""){
			alert("请先根据号牌号码、号牌种类查询登记信息和车辆参数信息！");
			return false;
		}
		//强制预约验证（按车牌号码方式）
		var yuyue = getIsyuyue(lshval, hphmval, hpzlval, dsrsfzmhmblack, ywlx, ywyy, '1', '3', '', '',clsbdh);
		//yuyue = '1+成功';
		if(yuyue == "error"){
			return false;
		}
	...
	if(confirm("此次以车牌号方式受理，是否确定受理?")){
		...
		if(arrs[0] == "1" && arrs[1] == "1"){
			...
			$("#hpzl").removeAttr("disabled");
			$("#drv_form").attr("action", path+"/wpz/wpz_editSlgVehXxcj_wpz.action");
