  ...
  <head>
	...
	<script type="text/javascript">
		...
		function windowopen(id){
			var info = window.open('<%=request.getContextPath()%>/wpz/wpz_initSlCx.action?id=' + id + '&cznr=ck' ,'info','width='+(window.screen.availWidth-10)+',height='+(window.screen.availHeight-30)+ ',top=0,left=0,toolbar=no,menubar=no,scrollbars=yes,resizable=yes,location=no,status=yes');
		}
		...
		//查询受理数据
		function check(){
			$("#esfrom").attr("action", "<%=request.getContextPath()%>/wpz/wpz_initSlCxList_wpz.action");
			$("#esfrom").submit();
		}
		//导出数据
		function exports(){
			...
			$("#esfrom").attr("action", "<%=request.getContextPath()%>/wpz/wpz_getSlgVehExport.action");
			$("#esfrom").submit();
		}
					...
					<form action="<%=request.getContextPath()%>/veh/veh_initSlCxList.action" method="post" id="esfrom">
						<table class="table1" width="100%" border="0"
							...
							<tr>
								...
								<td class="tdl" style="text-align: left;">
									<select name="ywlx" id="ywlx_dmz" style="width:160px;">
							...
							<tr>
								...
								<td class="tdl" style="text-align: left;">
									<input type="text" id="hphm" name="hphm" value="${hphm}" onkeyup="clearspace(this)" onblur="clearallspace(this)"/>
								</td>
								<td class="tds" style="text-align: right;">
									号牌种类&nbsp;
								</td>
								<td class="tdl" style="text-align: left;" >
									<select name="hpzl" id="hpzl_dmz" style="width:160px;">
										<option value="">======请选择======</option>
							...
							<tr>
								...
								<td class="tdl" style="text-align: left;">
									<input name="jbr" id="jbr" value="${jbr}" type="text" />
								</td>
								<td class="tds" style="text-align: right;">
									经办人部门&nbsp;
								</td>
								<td class="tdl" style="text-align: left;" >
									<select name="jbrbm" id="jbrbm" style="width:160px;">
										<option value="">======请选择======</option>
							...
							<tr>
								...
								<td class="tdl" style="text-align: left;">
									<input name="shr" id="shr" value="${shr}" type="text" />
								</td>
								<td class="tds" style="text-align: right">
									审核状态&nbsp;
								</td>
								<td class="tdl" style="text-align: left;">
									<select name="shzt" id="shzt" style="width:160px;">
										<option value="">======请选择======</option>
							...
							<s:iterator id="dbjgZjxxb" value="#request.dbjgZjxxbList" status="st">
								<tr class="<s:if test="#st.odd == false">altrow</s:if>">
									...
									<td id="${dbjgZjxxb.id}zt">						 		
										<s:if test="#dbjgZjxxb.shjg == 1"><font color="green">审核通过</font></s:if>
										<s:elseif test="#dbjgZjxxb.sh_jg == 2"><font color="red">审核未通过</font></s:elseif>
