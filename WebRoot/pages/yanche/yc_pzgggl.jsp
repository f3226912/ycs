...
<html>
		...
		<script type="text/javascript">
			...
			function initEdit2(type,dmlb,dmz,yeType){
				...
				if(yeType=="pz"){	//拍照规格
					...
					if(type==2){
						var returnValue=window.showModalDialog(uri,null,"dialogHeight:450px;dialogWidth:670px;status:no;scroll:yes;center:yes");
					}else{
						var returnValue=window.showModalDialog(uri,null,"dialogHeight:780px;dialogWidth:670px;status:no;scroll:yes;center:yes");
					}
					if(returnValue!=null && returnValue!="0" && returnValue!="" && typeof(returnValue)!="undefined"){
						location.reload();
					}
						...
						<table class="table1" width="100%" border="0"
							...
							<tr>
								<td class="tds" style="text-align: center;">
									...
									<input type="hidden" name="ly" id="ly" value="pzgg"/>
								</td>
							</tr>
							<tr>
	...
	<!-- 拍照规格--Div3==========================================================-->
				<div id="div3">
					<input type="button" onclick="initEdit2(2,'','','pz')" value="添加" class="bnt" />
						...
						<s:if test="#request.ycByTypeList.size > 0">
							<s:iterator id="pvp" value="#request.ycByTypeList" status="st">
								<tr class="<s:if test="#st.odd == false">altrow</s:if>">
									...
									<td>${pvp.dmsm}</td>
									<td>
										<a href="#" onclick="CyxmmlOrPzggml(2,'${pvp.dmz}','pzggml')">
									...
									<td><a href="#" onclick="CyxmmlOrPzggml(2,'${pvp.dmz}','pzggml')">示例图</a></td>
									<td>${pvp.zt=="0"?"隐藏":"正常"}</td>
									<td>${pvp.dmsx=="0"?"不许修改":"允许修改"}</td>
									<td>
									<s:if test='#session.userbean.name=="jst"'>
										<a href="#" onclick="initEdit2(1,'${pvp.dmlb}','${pvp.dmz}','pz')">修改</a>
									</s:if>
									<s:else>
