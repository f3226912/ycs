  ...
  <head>
	...
	<style type="text/css">
		...
		* {
		...
		td {
			background: #FFFFFF; font-size: 12px; height: 18px;
		}
		.datalist th{
		...
		#zhang{ width:133px; height: 52px; border:0px solid red;
				line-height: 58px;
				position: absolute;
				margin-top: 5px;
			}
	...
	<div id=admin_main style="WIDTH:98%; margin:0 auto; color: #000;">
		...
		<div class="div">
			<div style="font-size: 12px;">
				<p style="width: 100%; text-align: center;"><strong>校车查验记录表</strong></p>
				<p style="width: 100%; float: left">
					号牌号码<span style="font-size: 12px;">（流水号或其他与车辆能对应的号码）</span>：${request.cybdObj[5]}&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp
					校车种类：<input type="checkbox" <s:if test='#request.zyxc=="yes"'>checked="checked"</s:if> value="yes">专用校车 &nbsp&nbsp
							 <input type="checkbox" <s:if test='#request.zyxc=="no"'>checked="checked"</s:if> value="no">非专用校车
				</p>
		<table class="edittable" border="0" cellpadding="0" cellspacing="0" align="center">
				  <tr>
				    <td colspan="8">
						<strong>业务类型</strong>&nbsp&nbsp
						<input type="checkbox" <s:if test='#request.ywlxOryy=="AA"'>checked="checked"</s:if> value="AA">注册登记 &nbsp&nbsp
						<input type="checkbox" value="">转入&nbsp;&nbsp;
						<input type="checkbox" <s:if test='#request.ywlxOryy=="DH"'>checked="checked"</s:if> value="DH">更换整车&nbsp;&nbsp;
						<input type="checkbox" <s:if test='#request.ywlxOryy=="B"'>checked="checked"</s:if> value="B">转移登记&nbsp;&nbsp;
						<input type="checkbox" <s:if test='#request.ywlxOryy=="DP"'>checked="checked"</s:if> value="DP">变更迁出&nbsp;&nbsp;
						<input type="checkbox" <s:if test='#request.ywlxOryy=="DG"'>checked="checked"</s:if> value="DG">更换车身或者车架&nbsp;&nbsp;
						<input type="checkbox" value="">申请校车使用许可&nbsp;&nbsp;
						<input type="checkbox" value="">核发检验合格标志/期满换发校车标牌&nbsp;&nbsp;
						<input type="checkbox" value="">非专用校车不再作为校车使用
				   </td>
				  </tr>
				  <tr>
				  ...
				  <tr>
				    <td rowspan="9" style="text-align: center;">通用项目
				    <td style="text-align: center;">1</td>
				    <td width="140px;">车辆识别代号</td>
				    <td style="text-align: center;" width="80px;"><s:property value="#request.cyxMap.get(1)" /></td>
				    <td rowspan="6" style="text-align: center;" width="80px;">大中型客<br>车、危险<br>化学品运<br>输车等</td>
				    <td style="text-align: center;">16</td>
				    <td width="160px;">车身外观标识</td>
				  ...
				  <tr>
				    ...
				    <td width="160px;">照管人员座位</td>
				    <td style="text-align: center;" width="80px;">
				    	<s:property value="#request.cyxMap.get(17)" />
				    </td>
				  </tr>
				  <tr>
				  ...
				  <tr>				
				    ...
				    <td width="160px;">校车标牌</td>
				    <td style="text-align: center;" width="80px;"><s:property value="#request.cyxMap.get(21)"/></td>
				  </tr>
				  <tr>
				  ...
				  <tr>
					...
					<td width="140px;">急救箱</td>
					<td style="text-align: center;" width="80px;"><s:property value="#request.cyxMap.get(15)" /></td>
				  </tr>
				  <tr>
					<td colspan="5" style="text-align: center; padding-top: 2px;"  height="190px";>
					<img src="http://100.100.21.61/cmp_new/view_pic.asp?efid=${request.zpid}" id="ycImg" onload="adapt()" border="0">
					</td>
					<td colspan="3" style="height: 190px;">备&nbsp&nbsp注:<br/>
						<div style="height: 160px; width: 100%;">
							<br/>
							机动车所有人/申请人：
							<s:if test="#request.sqr.length()<8">
								${request.sqr}
							</s:if>
							<s:else>
								<div style="width: 100%; height:35px; text-align: center;">${request.sqr}</div>
							</s:else>
						</div>
						<p style="float: right;">&nbsp&nbsp&nbsp&nbsp年&nbsp&nbsp&nbsp&nbsp月&nbsp&nbsp&nbsp&nbsp日</p>
					</td>
				  ...
				  <tr>
					<td colspan="8" style="text-align: center; height:110px; padding-top: 2px;">
		...
		说明：1、填表时在对应的校车类型和业务类型名称上划“√”；2、对按照规定不须查验的项目，在对应的判定栏内划“—”；3、本表所列查验项目判定合格时在
		对应栏划“√”，判定不合格时在对应栏划“×”，本表以外的查验项目不合格时，在备注栏内注明情况，查验结论签注为“不合格”；所有查验项目合格，查验结论
		签注为“合格”；4、复检合格时，查验员签字并签注日期；复检仍不合格的，不签注；5、专用校车注册登记查验时，“车身颜色、核定载人数、车辆类型”判定
		栏内签注查验确定的相应内容；6、非专用校车申请校车使用许可查验时，按照幼儿校车、小学生校车、中小学生校车、初中生校车四种情形分别核定乘坐的学
		生数和成人数，并签注在备注栏内。
		</div>
			</div>
		</div>
