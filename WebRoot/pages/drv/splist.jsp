		...
		function delSlgxx(id){
			if(confirm("确定要删除吗?")){
				$.ajax({
					cache:false,
	    			async:false,
	    			url: '<%=request.getContextPath()%>/slgSpxx/slgSpxx_delSlgSpxx.action',
	    			type: 'post',
	    			data: {"id": id},
	    			dataType: 'json',
	    			error: function(){
    ...
    <div class="content1" style="width: 100%;">
		  <div class="roundedBox" id="type1" style="width: 95%;">
				<div class="right" style="width: 98%;">
					<form action="<%=request.getContextPath()%>/yujing/slgSpxx_initSlgSpxxList.action" method="post" id="esfrom">
					...
					<div style="width: 98%;height: 35px;padding-top: 10px;">
						<input class="bnt" type="button" value="添加" onclick="javascript:editsp('<%=request.getContextPath()%>/yujing/slgSpxx_selSlgSpxx.action?editType=add');" id="excelid" style="cursor:pointer;" />
					</div>
						...
						<s:if test="#request.slgSpxxList.size > 0">
							<s:iterator id="slgsp" value="#request.slgSpxxList" status="st">
								<tr class="<s:if test="#st.odd == false">altrow</s:if>">
									...
									<td id="${slgsp.id}">
										<a href="javascript: editsp('<%=request.getContextPath()%>/yujing/slgSpxx_selSlgSpxx.action?editType=query&id=${slgsp.id}');">查看</a>
										<c:if test="${slgsp.sprBmkj == kjbmid}">
											<a href="javascript: delSlgxx(${slgsp.id})" target="mainFrame">删除</a>
											<a href="javascript: editsp('<%=request.getContextPath()%>/yujing/slgSpxx_selSlgSpxx.action?editType=edit&id=${slgsp.id}');">修改</a>
										</c:if>
									</td>
								</tr>
							</s:iterator>
