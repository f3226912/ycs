		...
		<script type="text/javascript">
			...
			function submitfrom() {
				if('新增' == edittype){
					...
					if(idno != null && idno != ''){
						...
						if(sfzmmc == 'A'){
							...
							if(file1 == ""){
								...
								settab(this,0);
								return false;
							}else if(file1 != "C:\\ycszh_sfz1\\zp.bmp"){
								alert("驾驶人身份证图片路径读取失败!请读取后点击浏览选择C:\\ycszh_sfz1\\zp.bmp");
								settab(this,0);
								return false;
							}else{
					...
					//验证代办人身份证读取
					var idno2 = $("#slgDrvXxcjbidNo2").val();
					...
					//验证身份证明号码
					var slgDrvXxcjbsfzmhm = checknotnull2(document.getElementById("slgDrvXxcjbsfzmhm"),'请输入身份证明号码',1);
					if (slgDrvXxcjbsfzmhm != "true") {
						return false;
					}
					//验证姓名
					var slgDrvXxcjbxm = checknotnull2(document.getElementById("slgDrvXxcjbxm"),'请输入姓名',1);
				...
				drv_form.submit();
				/*var tyblsh = $("#slgDrvXxcjblsh").val();
				$.ajax({
					type:'get',
					url: '<%=request.getContextPath()%>/drv/getTyblsh.action',
					data:{lsh:tyblsh},//发送的参数
					dataType: 'html',
					success:function(data){
						if(data == 0){
							var imgurl = '<%=request.getContextPath()%>/artDialog4.1.6/skins/icons/loading2.gif';
		...
		<div class="div">
			...
			<div id="tableid0" class="tablist block">
				<table class="edittable" border="0" cellpadding="0" cellspacing="0" align="center">
					<tr>
						<td style="text-align: right;width: 10%">身份证明名称：</td>
					...
					<tr>
						...
						<td style="text-align: left;width: 23%" colspan="2">
							&nbsp;<input type="file" name="file1" class="disabled" id="file1" title="驾驶人身份证图片路径读取失败!请读取后点击浏览选择C:\\ycszh_sfz1\\zp.bmp" style="width:320px;"/>
						</td>
						<td style="text-align: right;width: 10%">&nbsp;照片路径：</td>
						<td style="text-align: left;width: 23%" colspan="2">
							&nbsp;<input type="file" name="file2" class="disabled" id="file2" title="驾驶人身份证图片路径读取失败!请读取后点击浏览选择C:\\ycszh_sfz2\\zp.bmp" style="width:320px;"/>
						</td>
					</tr>
				</table>
				<br />	  
			...
			<div id="tableid1" class="tablist">
				<table class="edittable" border="0" cellpadding="0" cellspacing="0" align="center">
					...
					<tr>
						<td style="text-align: right;width: 10%">身份证明号码：</td>
						...
						<td style="text-align: left;" colspan="3">
							...
							<input type="text" class="disabled1" id="slgDrvXxcjbxm"
								name="slgDrvXxcjb.xm" value="${slgDrvXxcjb.xm}" size="20"
								maxlength="20" />&nbsp;<font style="color: red;">*</font>
						</td>
					</tr>
					<tr>
