  ...
  <head>
	...
	<script type="text/javascript">
		...
		function windowopen(wwlsh,xg){
			window.open('<%=request.getContextPath()%>/ezxfw/ezxfw_initDrvChanInfo.action?wwlsh=' + wwlsh +'&xg='+xg,'info','width='+(window.screen.availWidth-10)+',height='+(window.screen.availHeight-30)+ ',top=0,left=0,toolbar=no,menubar=no,scrollbars=yes,resizable=yes,location=no,status=yes');
		}
		...
		function tqsh(){
			var hblxval=$("#hblxtq").val();
			if(hblxval==''){
				alert("请选择业务类型！");
				return false;
			}else{
				if(confirm("是否提取数据？")==true){
					window.hblxform.action='<%=request.getContextPath()%>/ezxfw/ezxfw_getTqdrvs.action';
    ...
    <div class="content1" style="width: 100%;">
		  <div class="roundedBox" id="type1" style="width: 95%;">
				<div class="right" style="width: 98%;">
					<form action="<%=request.getContextPath()%>/ezxfw/ezxfw_initDrvChanList.action" method="post" id="ezxfwform" name="ezxfwform">
						...
						<form action="" method="post" id="hblxform" name="hblxform">
							<td class="tds" style="text-align: right">
								...
								<td class="tdl" style="text-align: left;">
									<s:select list="#request.ywlxList" theme="simple"
											id="hblxtq"
											listKey="key" listValue="value" headerKey="" headerValue="---请选择---" name="hblxtq" value="#request.hblxtq"></s:select>&nbsp;
								</td>
							<td>
					...
					<table class="datalist" width="100%" border="0"
						...
						<s:if test="#request.ezDrvLiceChanAppList.size > 0">
							<s:iterator id="dc" value="#request.ezDrvLiceChanAppList" status="st">
