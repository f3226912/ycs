...
function windowopenview(fpzh,lx,sdate,edate,hblx) {
	window
			.open(
					'<%=request.getContextPath()%>/ezxfw/ezxfw_initDrvGzlView.action?fpzh=' + fpzh+'&lx='+lx+'&sdate='+sdate+'&edate='+edate+'&hblx='+hblx,
					'info',
					'width='
...
function exportToExport() {
	...
	if (nodata.length > 0) {
		...
		var uri = "<%=request.getContextPath()%>/ezxfw/ezxfw_ezxfwDrvGzlListExcel.action";
		...
		<div class="content1" style="width: 100%;">
			<div class="roundedBox" id="type1" style="width: 95%;">
				<div class="right" style="width: 98%;">
					<form
						action="<%=request.getContextPath()%>/ezxfw/ezxfw_initDrvGzlList.action"
						...
						<table class="table1" width="100%" border="0" cellpadding="0"
							...
							<tr>
								...
								<td class="tdl" colspan="3" style="text-align: left;">
									<input type="text" size="10" value="${q_account}" name="q_account"/>
								</td>
								<td class="tds" style="text-align: right">
									分配日期&nbsp;
								</td>
					...
					<div style="font-size:13px">
						<s:iterator id="sl" value="#request.slList" status="st">
							总待分配数量${sl[0]}宗，总待审核数量 ${sl[1]}宗，${s_date}至${e_date}已分配情况如下：
						</s:iterator>
					</div>
					<table class="datalist" width="100%" border="0" cellpadding="0"
						...
						<s:if test="#request.ezDrvLiceGzlList.size > 0">
							<s:iterator id="dc" value="#request.ezDrvLiceGzlList"
								status="st">
								<tr class="<s:if test="#st.odd == false">altrow</s:if>">
									...
									<td>
										<a href="javascript:void(0);" onclick="javascript:windowopenview('${dc[0] }','drv','${s_date}','${e_date}','${dc[7]}');">点击查看详细</a>
									</td>
