...
<script type="text/javascript">   
   $(document).ready(function(){
	   ...
	   $(".test").click(function(){
			var lsh=$.trim($(this).attr("lsh"));
			var showType=$.trim($(this).attr("showType"));
			//alert(lsh+","+showType);
			if(lsh!="" && lsh!=null && showType!="" && showType!=null ){
				var uri="<%=request.getContextPath()%>/qrelate/qrelate_showDetailInfoByLsh.action?lsh="+lsh+"&showType="+showType;
				window.showModalDialog(uri,null,"dialogHeight:380px;dialogWidth:460px;status:no;scroll:no;center:yes");  	
			}else{
				alert("该记录无流水号,无法查询详细!");	
			}
		});
   ...
   function query(){
		...
		var url="<%=request.getContextPath()%>/qrelate/qrelate_qrelate1.action";
	...
	<body>
	 <form id="queryForm" name="queryForm" method="post">
		<div style="width:95%;margin: 0 auto;">
		  ...
		  <div style="font-weight: bolder;color: red;">在线驾驶证补证换证业务数：${map.rscount}</div><br/>
		    <table class="datalist" id="tableList" width="100%" border="0" cellpadding="0" cellspacing="0">
			  ...
			  <s:iterator var="rl" value="#request.resultList">
			  	<tr>
		        	...
		        	<td>${rl[11]}</td>
		        	<td>
		        		<a href="#" class="test" showType="1" lsh="${rl[0]}">查看详细</a>
		        	</td>
		        </tr>
			  </s:iterator>
			  </s:if>
