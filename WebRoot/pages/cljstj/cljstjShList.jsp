...
<script type="text/javascript">
    ...
    function sh(xh){
    	window.location.href='<%=request.getContextPath()%>/cljstj/cljstj_cljstjShYm.action?xh='+xh+'&cz=1';
    }

	function zqsj(){
		window.location.href='<%=request.getContextPath()%>/cljstj/cljstj_cljstjHq.action?cz=1';
	}
	...
	<div class="content1" style="width:100%;">
		<div class="roundedBox" id="type1" style="width:95%;">
			<div class="right" style="width:90%;">
				<form action="<%=request.getContextPath()%>/cljstj/cljstj_cljstjQuery.action?cz=2" method="post" id="searchfromid">
				<table class="table1" width="100%" border="0" cellpadding="0" cellspacing="0">
					...
					<tr>
						<td class="tds" colspan="4" style="text-align: center;">
							<div align="center">
								<input type="button" id="frombutid" value="查  询" onclick="bd();" style="cursor:pointer;"/>
								&nbsp;&nbsp;
								<input type="button" value="抓取待审核数据" onclick="zqsj()"/>
								&nbsp;&nbsp;
					...
					<s:if test="tjList.size > 0">
						<s:iterator id="cljstj" value="tjList" status="st">
							<tr class="<s:if test="#st.odd == false">altrow</s:if>">
								...
								<td>
									<s:if test='#request.cljstj.zhclhj == "1"'>
										待受理
									</s:if>
									<s:elseif test='#request.cljstj.zhclhj == "888"'>
										已受理
									</s:elseif>
								</td>
								<td>
									<s:if test='#request.cljstj.zhclzt == "1"&&#request.cljstj.zhclhj == "888"'>
										成功
									</s:if>
									<s:elseif test='#request.cljstj.zhclzt == "0"&&#request.cljstj.zhclhj == "888"'>
										失败
									</s:elseif>
									<s:elseif test='#request.cljstj.zhclhj == "1"'>
										正在处理
									</s:elseif>
								</td>
								<td>
									<s:if test='#request.cz == "1"'>
										<a href="#" onclick="sh('<s:property value="#cljstj.sxh"/>')">【审核】</a>
									</s:if>
									<s:elseif test='#request.cz == "2"'>
										<a href="#" onclick="ck('<s:property value="#cljstj.sxh"/>')">【查询】</a>
									</s:elseif>
								</td>
							</tr>
						</s:iterator>
