...
<html>
     ...
     function checkFormVidate(){
		 ...
		 if(noPic==undefined){
			...
			if(noMacthLxs.length>0){
			    ...
			    if(checknotnull(lxdh,'请填写手机号码!')!= "true") {
			        return false;
			    }else{
  ...
  <body>
	<center>
		...
		<div id="admin_main" style="width: 98%;margin-top: 5px;overflow-y:scroll;overflow-x:none;">
			<form action="<%=request.getContextPath()%>/jsrdzda/jsrdzda_jxFirstCheck.action?type=jx" method="post" id="jsrdzform" name="jsrdzform" target="abc">
				...
				<table border="0" cellpadding="0" cellspacing="0" class="edittable"  style="width: 98%;overflow-y:scroll;overflow-x:none;">
					<tr>
						...
						<td class="trs" style="text-align:left;width:240px"><input type="hidden" name="dzxxb.ywlx" id="ywlx" value="${dzxxb.ywlx }"/>
						                                                    <input type="text" id="ywyy" value="${dzxxb.ywyy }"/><!-- 用业务原因代替业务类型翻译值 --> </td>
					...
					<tr>
						...
						<td class="trs" style="text-align:left;">&nbsp;
						            ...
						            <input type="radio" value="0" name="dzxxb.xb" <s:if test="dzxxb.xb ==1">checked="checked"</s:if> />女</td>
					</tr>
				    <tr>
						 ...
						 <table style="width: 100%">
						  <tr>
						    ...
						    <s:iterator id="pto" value="#request.photos" status="st">						     
						              <div style="overflow:none;display: block;width:410px;height:590px;border:1px solid <s:if test="#pto.xh!=null && #pto.xh!=''">#666</s:if><s:else>red</s:else>;">
						                 <s:if test="#pto.xh!=null && #pto.xh!=''">
							                 ...
							                 <s:if test="#request.ywlxZlSize!=0 ">
		                                        <div style="width100%;overflow: hidden;text-align: center;">		                                        
		                                            <input type="button" id="shBtn${pto.xh}" name="shBtn${pto.xh }" <s:if test="#pto.zt==0">value="不合格"</s:if><s:elseif test="#pto.zt=='JT'">value="合格"</s:elseif> onclick="openDialog(${pto.xh })" class="jsrbtn"/>
		                                        </div>
									         </s:if>
									              <div id="divMove${pto.xh }" style="height: 45px;width100%;overflow: hidden;text-align: left;">
									              </div>								                 
						...
						<td colspan="4">
						      <s:iterator id="otherPto" value="#request.noMatchList" status="st">
					               <div style="overflow:none;display: block;width:46%;height:520px;border:1px solid #666;float:left;margin-left:10px;margin-top:10px;">
					                 <s:if test="#otherPto.xh!=null && #otherPto.xh!=''">
			        ...
			        <tr>
			         ...
			         <tr>
			           <td style="width: 80px;text-align: center;" rowspan="4" >
					          <strong>退办原因</strong>
					          <input type="hidden" name="isAllYY" id="isAllYY" value=""/><!-- 勾选全部不合格时，存放退办原因key -->
					...
					<tr>
						<td colspan="5" height="50" align="center">
							<s:if test="#request.editType != '查看'">
			    				<input type="button" onclick="javascript:checkFormVidate();" value="保存" style="cursor:pointer;width: 79px;height: 28px;border: none;background: url(<%=request.getContextPath()%>/images/an2.gif) no-repeat;color: #FFFFFF;font-weight: bold;">
			    			</s:if>
			    			<input type="button" onclick="javascript:window.location.href='<%=request.getContextPath()%>/jsrdzda/jsrdzda_addCurrentLockList.action?btnType=back'" value="返 回" style="margin-left: 200px;cursor:pointer;width: 79px;height: 28px;border: none;background: url(<%=request.getContextPath()%>/images/an2.gif) no-repeat;color: #FFFFFF;font-weight: bold;">
						</td>
					</tr>
				</table>
			</form>
