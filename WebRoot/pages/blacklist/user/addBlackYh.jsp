...
<script type="text/javascript">   
   $(document).ready(function(){
		...
		if(editType == "查看"){
			...
			if(dqzt=='1'){
			  $("#sdyy").val('${hmdyh.sdyy}');
			  $("#sdyy").attr("disabled","disabled");
			  
			}else{
			  $("#jsyy").val('${hmdyh.jsyy}');
			  $("#jsyy").attr("disabled","disabled");
			}
			$("#sdbz").attr("disabled","disabled");
			
			var sdlx ='${hmdyh.sdlx}';
			if(sdlx=='1'){
			   $("input[type=radio]").attr("checked",sdlx);
			   var sdlxR =$('input[name="hmdyh.sdlx"]:checked').val();
			   $('#dialogTable').find('tr:eq(6)').show();
			}
		}
           //录入黑名单用户
        $('#addBtn').click(function(){
			...
			if(checknotnull(sfzmhm,'请填写身份证明号码!') != "true"){
			    ...
			    if(sfzmhm.value.length>20){
			      alert('身份证明号码的长度不能超过18!');
			      return false;
			    }
			}
           ...
           var url="<%=request.getContextPath()%>/blacklist/black_addHmdYh.action";
	...
	<br>
	 ...
	 <iframe id="abc" name="abc" width="0" height="0" style="display: none;"></iframe>
	      <input type="hidden" value="${editType} " name="editType" id="editType"/>
		   <table  class="table1" id="dialogTable"  width="420px" height="320px" align="center" border="0" cellpadding="0" cellspacing="0">
			  <tr>
			    <td class="tds" height="25" colspan="3">${editType}限制代办业务用户信息</td>
			  </tr>
			   <tr>
			    <td class="tds" style="text-align: right;"><font color="red">*</font>姓名：</td>
			    <td class="tds" style="text-align: left;" colspan="2"><input type="text" name="hmdyh.xm" id="xm" value="${hmdyh.xm}"/></td>
			  </tr>
			  <tr>
			    <td class="tds" style="text-align: right;"><font color="red">*</font>身份证明号码：</td>
			    <td class="tds" style="text-align: left;"  colspan="2"><input type="text" name="hmdyh.sfzmhm" id="sfzmhm" value="${hmdyh.sfzmhm}" maxlength="18"/></td>
			  </tr>
			  <s:if test="#request.hmdyh==null || #request.hmdyh.dqzt==1">
			      <tr>
			  ...
			  <s:else>  
	             <tr>
			      ...
			      <td class="tds" style="text-align: left;"  colspan="2">
			       <s:select theme="simple" name="hmdyh.jsyy" id="jsyy" list="#request.sjzdJsList" value="#request.hmdyh.dmms1" listKey="dmz" listValue="dmms1" headerKey="--" headerValue="--请选择--" />
	              </td>
	             </tr>
			  </s:else>
			  <tr>
			    ...
			    <td class="tds" style="text-align: left;"  colspan="2">
			       <textarea cols="30" rows="4" name="<s:if test="#request.hmdyh==null || #request.hmdyh.dqzt==1">hmdyh.sdbz</s:if><s:else>hmdyh.jsbz</s:else>" id="sdbz" ><s:if test="#request.hmdyh==null || #request.hmdyh.dqzt==1">${hmdyh.sdbz}</s:if><s:else>${hmdyh.jsbz}</s:else></textarea>
			   </td>
			  </tr>
			  <tr>
			  ...
			  <tr style="display: none;">
			    ...
			    <td class="tds" style="text-align: left;"  colspan="2"><input type="text" name="hmdyh.sdyxqz" id="sdyxqz"
			     class="Wdate" value="<s:date name="yxqzStr" format="yyyy-MM-dd"/>" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd',startDate:'%y-%M-%d',minDate:'%y-%M-%d'})" /></td>
			  </tr>  
			  <tr>
