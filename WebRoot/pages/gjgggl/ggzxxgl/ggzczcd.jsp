	...
	<body>
		  <div style="width:95%;" >
		    ...
		    <table id="showData" class="datalist" width="100%" border="0" cellpadding="0" cellspacing="0">
		    	  <tr>
		    	  	<td colspan="4" class="tr1" style="font-weight: bold;" >广告证错证重打</td>
		    	  </tr>
		    	  <tr>
				    <td class="tds1" style="text-align: right;width:80px;">流水号：</td>
				    <td class="tds2" style="text-align: left;" colspan="3">
				    	<form id="querySubmit" action="<%=request.getContextPath()%>/gjgg/ggz_getBusGgjgclsb_cd.action">
			    			<input type="text" name="lshStr" id="lshStr" value="${request.lsh}" />
				  ...
				  <tr>
				    ...
				    <td class="tds2" style="text-align: left;width:80px;">
				    	<!-- 广告喷涂业务流水号 -->
				    	<input type="hidden" id="lsh" value="${request.lsh}" />
				    	<!-- 广告喷涂业务对应的公交公司id -->
				    	<input type="hidden" id="gjgsid" value="${request.busGgjgclsb.gjgsid}" />
				    	${request.busVehicleBase.hphm}
				    	<!--  
				    	<input type="text" id="hphm" value="${request.busVehicleBase.hphm}" disabled="disabled"/>
				    	-->
				    </td>
				  ...
				  <tr>
				    ...
				    <td class="tds2" style="text-align: left;width:240px;">
				    	${request.gjgsmc}
				    	<!--  
		    			<input type="text" name="gjgsmc" id="gjgsmc" value="${request.gjgsmc}" disabled="disabled"/>
		    			-->
				    </td>
				     ...
				     <td class="tds2" style="text-align: left;width:80px;">
				     	...
				     	<!--  
				    	<input type="text" name="busLine" id="busLine" value="${request.busVehicleBase.busLine}" disabled="disabled" />
				    	-->
				     </td>
				  ...
				  <tr>
				    ...
				    <td class="tds2" style="text-align: left;width:240px;">
					    ...
					    <!--  
		    			<input type="text" name="clsbdh" id="clsbdh" value="${request.busVehicleBase.clsbdh}" disabled="disabled"/>
		    			-->
				    </td>
				    ...
				    <td class="tds2" style="text-align: left;width:240px;">
				    	${request.busVehicleBase.clxh}
				   	    <!--  
		    			<input type="text" name="clxh" id="clxh" value="${request.busVehicleBase.clxh}" disabled="disabled"/>
		    			-->
				    </td>
				  </tr>
				  <tr>
				  ...
				  <tr>
				    <td class="tds" style="text-align: center;" colspan="4">
				    	<input id="showPicArea" type="button" class="bnt"  style="width: 80px;" onclick="showPrintAreaToShow();" isShow="false"  value="广告证预览" <s:if test='#request.isFirst=="false" || #request.isFirst=="" '>disabled="disabled"</s:if> />
					    <input  id="printButton" type="button" class="bnt"  style="width: 80px;" onclick="printPic();" value="打印" <s:if test='#request.isFirst=="false" || #request.isFirst=="" || (#request.queryResult!=null && #request.queryResult!="" ) '>disabled="disabled"</s:if> />
				    </td>
				  </tr>
			</table>
...
<script type="text/javascript">
	...
	function addBusCertifyCzcd(){
		var lsh=$.trim($("#lsh").val());
		if(typeof(lsh)!="undefined" &&　lsh!=null && lsh!=""){
			$.ajax({
				url: "<%=request.getContextPath()%>/gjgg/ggz_addBusCertifyCzcd.action",
	  			type:'post',
	  			cache: false,
	  			async: true,
	  			dataType:'text',
