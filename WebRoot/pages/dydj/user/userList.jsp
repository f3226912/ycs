...
<script type="text/javascript">   
   $(document).ready(function(){
        $('#addBtn').click(function(){
           window.location.href="<%=request.getContextPath()%>/dydj/dydj_initUser.action";
        });
        $('#queryBtn').click(function(){
             var  url="<%=request.getContextPath()%>/dydj/dydj_getUsersList.action";
 ...
 // 冻结/活动用户
  function freezeUser(id){
     		$.ajax({
			type:'POST',
			url: '<%=request.getContextPath()%>/dydj/dydj_freezeUser.action?temp=cgAndYz',
			data:{'id':id},//发送的参数
			dataType: 'html',
  ...
  function checkPwd(id){
	 $.ajax({
		...
		cache : false,
		type:'POST',
		url: '<%=request.getContextPath()%>/dydj/dydj_reSetPwd.action?temp=cgAndYz',
		data:{id:id}, //发送的参数
		dataType: 'html',
		success:function(data){
			if (data == 1) {
	...
	<body>
		...
		<div style="width:90%;margin: 0 auto;">
		   <table  class="table1"  width="100%" align="center" border="0" cellpadding="0" cellspacing="0">
			  <tr>
			    <td class="tds" height="25" colspan="4" >邮政、车管用户信息查询</td>
			  </tr>
			  <tr>
